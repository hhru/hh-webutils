...
class TestPageHelpersPaging(LxmlTestCaseMixin, unittest.TestCase):
    ...
    def get_dots_pages(paging):
        dots_pages = []
        for item in paging['pages']:
            if item.get('text') == "...":
                dots_pages.append(item.get('page'))
    ...
    def test_no_pages(self):
        current_page, paging = ph.get_paging(logger, items_number=10)
        self.assertEqual(current_page, 0)
        self.assertEqual(paging, None)

        current_page, paging = ph.get_paging(logger, items_number=100, items_on_page=100)
        self.assertEqual(current_page, 0)
        self.assertEqual(paging, None)

        current_page, paging = ph.get_paging(logger, items_number=10, current_page=1)
        self.assertEqual(current_page, 0)
        self.assertEqual(paging, None)

        current_page, paging = ph.get_paging(logger, items_number=10, items_on_page=10, current_page=10)
        self.assertEqual(current_page, 0)
        self.assertEqual(paging, None)
    ...
    def test_bad_items_number(self):
        self.assert_total_error(*ph.get_paging(logger, items_number='100ds'))

    def test_bad_items_on_page(self):
        self.assert_total_error(*ph.get_paging(logger, items_number='100', items_on_page='0s'))

    def test_zero_items_on_page(self):
        self.assert_total_error(*ph.get_paging(logger, items_number='100', items_on_page='0'))

    def test_negative_items_on_page(self):
        self.assert_total_error(*ph.get_paging(logger, items_number='100', items_on_page='-1'))

    def test_bad_total_pages(self):
        current_page, paging_xml = ph.get_paging(logger, total_pages='50abcd')
        self.assertEqual(current_page, None)
        self.assertEqual(paging_xml, None)

    def test_both_total_pages_and_items_number_specified(self):
        try:
            ph.get_paging(logger, total_pages=10, items_number=20)
    ...
    def test_input_params_to_int(self):
        current_page, paging = ph.get_paging(logger, items_number='100', current_page='2', items_on_page='10')
        self.assertEqual(current_page, 2)
        self.assertNotEqual(paging, None)

    def test_selected_page_in_dict(self):
        current_page, paging = ph.get_paging(logger, items_number=100)
        self.assertEqual(current_page, 0)
        self.assert_selected_page(paging.get('pages'), 0)

        current_page, paging = ph.get_paging(logger, items_number=10, items_on_page=3, current_page=100)
        self.assertEqual(current_page, 3)
        self.assert_selected_page(paging.get('pages'), 3)

    def test_pager_items_count(self):
        current_page, paging = ph.get_paging(logger, items_number=101, items_on_page=10)
        self.assertEqual(len(paging['pages']), 11)

        current_page, paging = ph.get_paging(logger, items_number=100, items_on_page=10)
        self.assertEqual(len(paging['pages']), 10)

        current_page, paging = ph.get_paging(logger, items_number=91, items_on_page=10)
        self.assertEqual(len(paging['pages']), 10)

        current_page, paging = ph.get_paging(logger, items_number=95, items_on_page=10)
        self.assertEqual(len(paging['pages']), 10)

    def test_dots(self):
        for cur_page in range(90, 94):
            current_page, paging = ph.get_paging(logger,
                                                 items_number=100,
                                                 items_on_page=1,
    ...
    def test_intervals_intersection(self):
        for cur_page in range(6):
            current_page, paging = ph.get_paging(logger, items_number=100, items_on_page=1,
        ...
        current_page, paging = ph.get_paging(logger, items_number=100, items_on_page=1, current_page=6,
                                             paging_links_number=10)
        self.assertEqual(self.get_pages_items(paging)[0].get('page'), 1)
        self.assertEqual(self.get_pages_items(paging)[-1].get('page'), 11)

        current_page, paging = ph.get_paging(logger, items_number=100, items_on_page=1, current_page=93,
                                             paging_links_number=10)
    ...
    def test_next_previous(self):
        current_page, paging = ph.get_paging(logger, items_number=10, items_on_page=1,
    ...
    def test_paging(self):
        current_page, paging = ph.get_paging(logger, items_number=10, items_on_page=3, current_page=1)
    ...
    def test_paging_with_total_pages(self):
        current_page, paging = ph.get_paging(logger, total_pages=4, items_on_page=3, current_page=3)
        expected = {
            'previous': {
                'disabled': False,
                'page': 2
